...
from headroom.providers.qwen import (
    CODING_PLAN_API_URL as _QWEN_CODING_PLAN_API_URL,
    TOKEN_PLAN_API_URL as _QWEN_TOKEN_PLAN_API_URL,
    api_key_env_for_plan as _qwen_api_key_env,
    build_launch_env as _build_qwen_launch_env,
    default_api_url as _qwen_default_api_url,
)
...
@click.option(
    ...
    "no_rtk",
    is_flag=True,
    help="Skip CLI context-tool setup (no effect for qwen)",
)
...
@click.option(
    "--plan",
    type=click.Choice(["coding", "token"], case_sensitive=False),
    default="coding",
    help=(
        "Qwen subscription plan — selects the upstream API and API-key env var.  "
        "'coding' uses BAILIAN_CODING_PLAN_API_KEY (default); "
        "'token' uses DASHSCOPE_API_KEY."
    ),
)
@click.option(
    "--qwen-api-url",
    default=None,
    help=(
        "Upstream Qwen API endpoint (auto-selected by --plan unless overridden).  "
        f"Coding: {_QWEN_CODING_PLAN_API_URL}  |  "
        f"Token: {_QWEN_TOKEN_PLAN_API_URL}"
    ),
)
@click.option("--prepare-only", is_flag=True, hidden=True)
...
def qwen(
    ...
    """Launch Qwen Code through Headroom proxy.

    \b
    Sets OPENAI_BASE_URL to route Qwen Code's OpenAI-compatible traffic
    through Headroom. The API key is read from the plan-specific env var
    and forwarded upstream as a Bearer token.

    \b
    Two plans are supported:
      --plan coding  (default) → BAILIAN_CODING_PLAN_API_KEY
      --plan token             → DASHSCOPE_API_KEY

    \b
    Examples:
        headroom wrap qwen                              # Coding plan (default)
        headroom wrap qwen --plan token                  # Token plan
        headroom wrap qwen -- -m qwen3-coder-plus       # Pass args to qwen
        headroom wrap qwen --port 9999                   # Custom proxy port
        headroom wrap qwen --qwen-api-url https://...    # Custom upstream
    """
    if prepare_only:
        return
    ...
    env, env_vars_display = _build_qwen_launch_env(
        port, os.environ, project=_project_name_from_cwd()
    )
